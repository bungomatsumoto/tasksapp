...
RSpec.describe Task, type: :model do
  ...
  it "search_title_status使用時、該当titleが一つ＆該当statusが2つある場合に一つ表示" do
    @task1 = Task.create(id: 1, title: 'GGG', explanation: '失敗テスト', status: 'wait')
    @task2 = Task.create(id: 2, title: 'GGG', explanation: '失敗テスト', status: 'running')
    @task3 = Task.create(id: 3, title: 'QQQ', explanation: '失敗テスト', status: 'running')
    @task4 = Task.create(id: 4, title: '55555', explanation: '失敗テスト', status: 'completed')
    expect(Task.search_title_status("QQQ", 1)).to include(@task3)
  end

  it "search_title_status使用時、該当titleが2つ＆該当statusが2つある場合に二つ表示" do
    @task1 = Task.create(id: 1, title: 'GGG', explanation: '成功テスト', status: 'wait')
    @task2 = Task.create(id: 2, title: 'GGG', explanation: 'テスト', status: 'wait')
    @task3 = Task.create(id: 3, title: 'QQQ', explanation: '失敗テスト', status: 'running')
    @task4 = Task.create(id: 4, title: '55555', explanation: '失敗テスト', status: 'completed')
    expect(Task.search_title_status("GGG", 0)).to include(@task1, @task2)
  end
